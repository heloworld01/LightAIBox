...
- **内置对话**：应用内即含微信式**对话**页，流式回复、Markdown + 离线 MathJax 渲染 LaTeX 公式、每条回复可「原文 / 渲染」切换、带时间分隔条（详见[内置对话](#内置对话)）。
...
- **实时流式**：回复边生成边刷进气泡；「停止」按钮可随时取消本次生成。
- **Markdown + LaTeX**：回复按 Markdown 渲染，公式由**本地打包的 MathJax v3（完全离线）**
  排版，支持 `\(...\)` / `\[...\]`（也支持 `$...$` / `$$...$$`）。围栏语言声明为
  `latex` / `tex` / `math` 的代码块会被渲染成居中公式；其余代码围栏（如 `python`）
  仍显示为带样式的代码块。
- **原文 / 渲染切换**：每条助手回复下方有链接，可在原始 markdown 与渲染视图间切换。
- **主题自适应**：气泡 / 头像 / 时间颜色随暗 / 亮主题切换。
...
└── ui/              # PySide6 界面
    ...
    └── resources/chat/  # 对话容器 HTML + 本地打包 MathJax v3（离线）
```
