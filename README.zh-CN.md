...
- **悬浮 Provider 面板**：独立、总在最前的半透明面板，悬浮于屏幕底部，随时列出全部「运行中」的 provider 及其剩余配额；主窗口最小化到托盘时依然可见。可拖到任意位置，点挂锁按钮即可锁定防误拖。开关位于标签栏右上角，状态跨重启记忆。
...
└── ui/              # PySide6 界面
    ...
    ├── providers_bar.py # 悬浮置顶面板：运行中 Provider + 用量/余量
